...
static void meson_vpu_vsync_rdma_irq(void *arg)
{
	...
	if (drm_vsync_rdma_handle[MESON_VPP0] == -1)
		return;

	//vpu_pipeline_check_finish_reg(vpp_index);
	vpu_pipeline_set_rdma_count(MESON_VPP0);
}
...
static void meson_vpu1_vsync_rdma_irq(void *arg)
{
	...
	if (drm_vsync_rdma_handle[MESON_VPP1] == -1)
		return;

	//vpu_pipeline_check_finish_reg(vpp_index);
	vpu_pipeline_set_rdma_count(MESON_VPP1);
}
...
static void meson_vpu2_vsync_rdma_irq(void *arg)
{
	...
	if (drm_vsync_rdma_handle[MESON_VPP2] == -1)
		return;

	//vpu_pipeline_check_finish_reg(vpp_index);
	vpu_pipeline_set_rdma_count(MESON_VPP2);
}
