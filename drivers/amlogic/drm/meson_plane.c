				    ...
				    struct drm_plane_state *state)
{
	...
	/*for async commit,  mif register should be flushed always*/
	plane_info->force_flush = state->state->legacy_cursor_update;
	DRM_DEBUG("plane_info->force_flush-%d\n", plane_info->force_flush);
